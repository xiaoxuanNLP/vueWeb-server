...
module.exports = {
  ...
  Login: async ctx => {
    ...
    let { userName, password,user_kind } = ctx.request.body;

    // 校验用户信息是否符合规则
    if (!checkUserInfo(ctx, userName, password,user_kind)) {
      return;
    }

    // 连接数据库根据用户名和密码查询用户信息
    let user = await userDao.Login(userName, password,user_kind);
    // 结果集长度为0则代表没有该用户
    if (user.length === 0) {
      ctx.body = {
        code: '004',
...
if(user_kind==='buyer'){
   ...
   * @param {Object} ctx
   */
  miniProgramLogin: async ctx => {  //UNDO TODO
  ...
  FindUserName: async ctx => {
    let { userName,user_kind } = ctx.request.body;

    // 校验用户名是否符合规则
    if (!checkUserName(ctx, userName,user_kind)) {
      return;
    }
    // 连接数据库根据用户名查询用户信息
  ...
  Register: async ctx => {
    ...
    try {
      // 连接数据库插入用户信息
      let registerResult = await userDao.Register(userName, password,user_kind);
      // 操作所影响的记录行数为1,则代表注册成功
      if (registerResult.affectedRows === 1) {
        ctx.body = {
          code: '001',
