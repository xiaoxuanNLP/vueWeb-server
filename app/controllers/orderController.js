...
module.exports = {
   ...
   */
  GetOrder: async ctx => {
    let { user_id,user_kind } = ctx.request.body;
    // 校验用户是否登录
    if (!checkLogin(ctx, user_id)) {
      return;
    }
    // 获取所有的订单id
    const ordersGroup = await orderDao.GetOrderGroup(user_id,user_kind);
  ...
  AddOrder: async (ctx) => {
    ...
    const orderID = +("" + user_id + timeTemp);
     ...
     for(let i=0;i<products.length;i++){
       const temp = products[i].productID;
       const seller_id = await productDao.GetSellerID(temp);
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const seller_id = seller_ids[i];
      let product = [orderID, user_id,seller_id, temp.productID, temp.num, temp.price, timeTemp,false];
      data.push(...product);
    }
