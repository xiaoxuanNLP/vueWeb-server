...
module.exports = {
  ...
  GetCollect: async (user_id) => {
    const sql = 'select * from collect where buyer_id=?';
    return await db.query(sql, user_id);
  },
  // 连接数据库,获取用户的某个收藏商品信息
  FindCollect: async (user_id, product_id) => {
    const sql = 'select * from collect where buyer_id=? and product_id=?';
    return await db.query(sql, [user_id, product_id]);
  },
  // 连接数据库,删除用户的某个收藏商品信息
  DeleteCollect: async (user_id, product_id) => {
    const sql = 'delete from collect where buyer_id=? and product_id=?';
    return await db.query(sql, [user_id, product_id]);
  }
}
